...
def parse_sql_file(file_path):
    ...
    table_name_search = re.search(
        r"CREATE TABLE \[dbo\]\.\[(\w+)\]", data, re.IGNORECASE
    )
    table_name = table_name_search.group(1) if table_name_search else None

    # Extract columns and data types
    columns = re.findall(r"\[(\w+)\] (\w+\(\d+(,\d+)?\))", data)

    # Try to extract foreign key constraints
    fk_constraints_search = re.findall(
        r"FOREIGN KEY \(\[(\w+)\]\) REFERENCES \[dbo\]\.\[(\w+)\] \(\[(\w+)\]\)",
        data,
        re.IGNORECASE,
    )
...
def parse_sql_string(data):
    ...
    # Extract columns and data types
    columns = re.findall(r"(\w+)\s+(\w+\(?\d*\)?)", data)
